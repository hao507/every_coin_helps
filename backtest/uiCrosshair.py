...
class Crosshair(QtCore.QObject):
    ...
    def __init__(self,parent,master):
        ...
        self.yAxises    = [0 for i in range(2)]
        self.leftX      = [0 for i in range(2)]
        self.showHLine  = [False for i in range(2)]
        self.textPrices = [pg.TextItem('',anchor=(1,1)) for i in range(2)]
        self.views      = [parent.centralWidget.getItem(i+1,0) for i in range(2)]
        self.rects      = [self.views[i].sceneBoundingRect() for i in range(2)]
        self.vLines     = [pg.InfiniteLine(angle=90, movable=False) for i in range(2)]
        self.hLines     = [pg.InfiniteLine(angle=0,  movable=False) for i in range(2)]
    ...
    def plotInfo(self,xAxis,yAxis):        
        ...
        try:
            ...
            tickDatetime    = data['candle_begin_time']
            openPrice       = data['open']
        ...
        if not (isinstance(tickDatetime,pd.datetime)):
            ts = (tickDatetime - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
            tsl = dt.datetime.utcfromtimestamp(ts)
            datetimeText = pd.datetime.strftime(tsl,'%Y-%m-%d %H:%M:%S')
            dateText     = pd.datetime.strftime(tsl,'%Y-%m-%d')
            timeText     = pd.datetime.strftime(tsl,'%H:%M:%S')
        else:
            datetimeText = ""
            dateText     = ""
            timeText     = ""
        ...
        bottomAxisHeight = self.views[1].getAxis('bottom').height()
        offset = QtCore.QPointF(rightAxisWidth,bottomAxisHeight)

        # 各个顶点
        tl = [self.views[i].vb.mapSceneToView(self.rects[i].topLeft()) for i in range(2)]
        br = [self.views[i].vb.mapSceneToView(self.rects[i].bottomRight()-offset) for i in range(2)]
        ...
        #self.__textSubSig.setPos(br[1].x(),tl[1].y())
