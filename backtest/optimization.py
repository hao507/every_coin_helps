...
from strategy import boll_bbw_bbpb
from strategy import bulin_K

# =====寻找最优参数
# 导入数据
all_data = pd.read_hdf(utils.project_path()+'/data/bitfinex_dataLTC.h5', key='all_data')
# 转换数据周期
rule_type = '5T'
...
# 构建参数候选组合 bulin
m_list = np.arange(25, 500, 25)
n_list = np.arange(0.25, 4, 0.25)
#bulin_k
h_list = np.arange(3,10,1)
k_list = np.arange(-0.2,0.2, 0.001)
#测试参数
# para = [425, 1.75, 5, 2]
# n_list=m_list=[1]
